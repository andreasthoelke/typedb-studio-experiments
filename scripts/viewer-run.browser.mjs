...
try {
 ...
 const schema='define attribute person-id, value string; attribute name, value string; entity person, owns person-id @key, owns name;';
 ...
 const schemaPage=await context.newPage();await schemaPage.goto(origin+'/schema?nvim=1');
 await schemaPage.waitForFunction(id=>{const b=window.ng?.getComponent(document.querySelector('ts-schema-page'))?.bridge;return b?.lastRequest?.id===id&&!b.pending;},result.id);
